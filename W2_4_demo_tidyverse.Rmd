...
```{r}
now <- Sys.time() ## info of time now

later <- now + 10000

time_difference <- difftime(later,now) ## the difference between then and now
```
...
lead(numbers,n = 2) ## starts at three
...
```{r}
wildschwein$timelag  <- as.numeric(difftime(lead(wildschwein$DatetimeUTC), wildschwein$DatetimeUTC)) ## time differences between two fixes; sampling - regular (every 15min) or iregular (3min,5min...)
...
```{r}
summarise(wildschwein, mean = mean(timelag, na.rm = T)) ## code is read from inside out
```
